...
Ghost::Ghost(int x, int y, int width, int height, ofImage spriteSheet, EntityManager* em, string color): Entity(x, y, width, height){
    ...
    if(color == "red"){
        ...
        sprite.load("images/PeekABoo.png"); //peekaboo is just another orange ghost for now
    }
    else if(color == "random"){
        sprite.load("images/Rando.png");
...
EntityManager* Ghost::getEM(){
    return this->em;
}

Animation* Ghost::getKillableAnim(){
    return killableAnim;
}